...
app.post("/generate", (req, res) => {
  ...
  const name = generateRandomBandName();
  res.render("index.ejs", {
    bandName: name,
  });
});
...
function generateRandomBandName() {
  ...
  const nounSize = nouns.length;
  const randomAdjPos = Math.floor(Math.random() * adjSize);
  const randomNounPos = Math.floor(Math.random() * nounSize);
  const adj = adjectives[randomAdjPos];
  const noun = nouns[randomNounPos];
  return adj + " " + noun;
}
